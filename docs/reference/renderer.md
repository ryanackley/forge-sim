...
This applies to **server mode** only (the Node-side reconciler used by tests and MCP). In **browser mode** (`forge-sim dev`), effects run in the browser exactly like real Forge — no wait helpers needed.
