...
External auth reads `providers.auth` and `remotes` from your `manifest.yml`. Provider client secrets are stored per-project in `<app>/.forge-sim/providers.json` (mode 0600). See [Talking to third-party APIs](../local-development/third-party-apis.md) for details.
